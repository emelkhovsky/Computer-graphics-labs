...
public:
	...
	virtual QImage calculateNewImagePixMap(const QImage &photo, int radius) = 0;//для работы с пикселями для всех фильтров, считаем новую картинку и возвращаем ее
};
...
public:
	...
	QImage calculateNewImagePixMap(const QImage &photo, int radius);
	QColor calculateNewPixelColor(QImage photo, int x, int y, int radius);//новый цвет для отдельного пикселя
};
//----------------------------------------Blur--------------------------------
